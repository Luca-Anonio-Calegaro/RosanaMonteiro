<?php
	...
	$perfis = array(1 => 'Publico', 2 => 'Usuário');
...
<?php
    ...
    $linhas_texto = str_replace("
", '', $linhas_texto);
    if($linhas_texto === ''){
        $arquivo = fopen("public/dados.hd", "w");
        fclose($arquivo);
        if($_SESSION['recarregar_paagina_dados'] === 'sim'){
            $_SESSION['recarregar_paagina_dados'] = 'nao';
            header("Refresh: 0");
        }
    }
?>
...
<?php
	...
	$linhas_file_adm = file("public/adm_validador.hd");
	fwrite($arquivo_adm, 'no_adm');
	fclose($arquivo_adm);
?>
<?php
    //abrindo o arquivo
	$arquivo_reca = fopen('public/recarregar_validador.hd', 'a');
	$linhas_file_reca = file("public/recarregar_validador.hd");
	unset($linhas_file_reca[1]);
    $arquivo_reca = fopen("public/recarregar_validador.hd", "w");
	$linhas_file_reca = file("public/recarregar_validador.hd");
	fwrite($arquivo_reca, 'no_adm');
	fclose($arquivo_reca);
?>
<?php
    //abrindo o arquivo
...
?>
<?php
    $login_erro = $_GET['login'];
    if($login_erro === 'erro'){
        echo "<script>alert('Ops! Não conseguimos fazer seu login. Por favor, tente novamente mais tarde!')</script>";
    }
?>
<?php
    if($_SESSION['linhas_texto_hd'] != false){
        //reseta os arquivos
        $arquivo_hd = fopen("public/dados_hd.hd", "w");
        fclose($arquivo_hd);
        //escreve os outros comentarios
        $arquivo_hd = fopen("public/dados_hd.hd", "a");
        fwrite($arquivo_hd, $_SESSION['linhas_texto_hd']);
        fclose($arquivo_hd);
        $_SESSION['linhas_texto_hd'] = false;
    }
?>
<!DOCTYPE html>
<html lang="pt">
    ...
    <body>
        ...
        <header>
            <nav class="navbar navbar-expand-sm navbar-dark bg-dark">
                ...
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item">
                        <a type="button" class="btn btn-outline-light" href="index.adm.php">Entrar</a>
                    </li>
                </ul>
                </div>
            </nav>
        ...
        <section>
            ...
            <div class="container bg-light">
                ...
                <!--FORMULÀRIO-->
                <div class="hide" id="div-tabela" style="background: linear-gradient(50deg, #50ff40, #02ffc0); width: 100%;"> 
                    <div class="card pb-2" style="background: linear-gradient(50deg, #50ff40, #02ffc0); width: 100%; height: 580px !important;">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col">
                                    <form method="post" action="public/registra_chamado.php" class="main">
                                        <div class="form-group">
                                            <label>Título</label>
                                            <input required name="titulo" type="text" class="form-control" placeholder="Digite seu título" aria-errormessage="Título nessesário para completar o comentário">
                                        </div>  
                                        <div class="form-group">
                                            <label>Categoria</label>
                                                <select name="categoria" class="form-control">
                                                <option>Categoria não escolhida</option>
                                                <option>Consulta</option>
                                                <option>Site</option>
                                                <option>Redes Sociais</option>
                                                <option>Outro</option>
                                                <!--<option>Ajuda</option> Mais para frente-->
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Descrição</label>
                                            <textarea required placeholder="Digite sua descrição" minlength="1" maxlength="160" rows="3" name="descricao" class="form-control" rows="3" style="min-height: 5.4rem !important; max-height: 17rem !important;" aria-errormessage="Descrição nessesária para completar o comentário"></textarea>
                                        </div>
                                        <? if($_SESSION["id"] != '') { ?>
                                            <div class="form-group">
                                                <label>Seu nome (opcional)</label>
                                                <input name="user" type="text" class="form-control" placeholder="Digite seu nome (opcional)">
                                            </div>
                                        <? } else {?>
                                            <div class="form-group d-flex">
                                                <div class="form-group col-md-6">
                                                    <label>Email</label>
                                                    <input required name="email" type="email" class="form-control" placeholder="Digite seu email" aria-errormessage="Email nessesário para exclusão de comentários">
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label>Seu nome (opcional)</label>
                                                    <input name="user" type="text" class="form-control" placeholder="Digite seu nome (opcional)">
                                        ...
                                        <div class="row mt-5">
                                            <div class="col-6">
                                                <button style="background: linear-gradient(50deg, #ff4059, #6c02ff) !important;" class="btn btn-lg input-enviar btn-block" type="submit">Enviar</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
        ...
        <section>
            <!--COMENTÁRIOS-->
            <div class="container bg-light pb-3" id="div-pai-comentarios">
